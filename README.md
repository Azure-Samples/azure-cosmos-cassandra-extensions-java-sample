...
In this code sample, we implement the [Azure Cosmos DB extension for Cassandra Retry Policy](https://github.com/Azure/azure-cosmos-cassandra-extensions) and build this using jitpack.io (see pom.xml). The retry policy handles onRequestErrors such as OverLoadedError, and parses the exception message to use RetryAfterMs field provided from the server as the back-off duration. If RetryAfterMs is not available, we default to an exponential growing back-off scheme. In this case the time between retries is increased by a growing back off time (default: 1000 ms) on each retry, unless maxRetryCount is -1, in which case we back-off with fixed duration. This a similar approach to that implemented in the built-in getRetryAfterInMilliseconds() method in the Java SDK for [Azure Cosmos DB SQL API](https://docs.microsoft.com/en-us/azure/cosmos-db/sql-api-java-get-started). It is important to handle rate limiting in Azure Cosmos DB.
